...
def make_cmsrun_config(config_file, **kwargs):
    ...
    # note: while the number of entries argument works for local testing,
    #       it seems to be overwritten and/or completely ignored by CRAB.
    #       to process files only partially to get smaller jobs,
    #       the best option seems to be to use EventAwareLumiBased splitting
    #       rather than FileBased splitting; see the crab config file.
    argparser = "import os\n"
    argparser += "import sys\n\n"
    argparser += "# Command line argument parser\n"
    argparser += "# generated by make_cmsrun_config.py\n"
    argparser += "entries = -1\n"
    argparser += "for arg in sys.argv:\n"
    argparser += "    if not '=' in arg: continue\n"
    ...
    argparser += "print(f'cmsRun {os.path.basename(__file__)}: found following raw sys.argv:')\n"
    argparser += "print(f'{sys.argv}')\n"
